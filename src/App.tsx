...
function useSupabaseData() {
  ...
  const updateAccount = async (identifier, updates) => {
    setAccounts(prev => prev.map(a => (a.id === identifier || a.name === identifier) ? { ...a, ...updates } : a));
    if (identifier && String(identifier).length > 10) {
      try { await fetchSupabase(`accounts?id=eq.${identifier}`, "PATCH", updates); }
      catch (e) { console.error("Update account failed", e); }
    }
  };

  const addTransaction = async (newTx) => {
    const tempId = `temp_${Date.now()}`;
    const optimistic = { ...newTx, id: tempId };
    setTransactions(prev => [optimistic, ...prev]);
    try {
      const result = await fetchSupabase("transactions", "POST", [newTx]);
      if (result?.[0]) {
        setTransactions(prev => prev.map(t => t.id === tempId ? result[0] : t));
      }
    } catch (e) { console.error("Add transaction failed", e); }
  };

  return { accounts, transactions, loading, updateTransaction, updateAccount, addTransaction };
}
...
const EXPENSE_CATEGORIES = [
  { id: 'dining',       label: '餐饮', emoji: '🍴', color: '#f59e0b', bg: '#fff7e6' },
  { id: 'shopping',     label: '购物', emoji: '🛍️', color: '#eb2f96', bg: '#fff0f6' },
  { id: 'transport',    label: '交通', emoji: '🚗', color: '#1677ff', bg: '#e6f4ff' },
  { id: 'subscription', label: '订阅', emoji: '📱', color: '#722ed1', bg: '#f9f0ff' },
  { id: 'investment',   label: '理财', emoji: '📈', color: '#1677ff', bg: '#e6f4ff' },
  { id: 'transfer',     label: '转账', emoji: '🔄', color: '#ff8c00', bg: '#fff7e6' },
  { id: 'education',    label: '教育', emoji: '🎓', color: '#10b981', bg: '#ecfdf5' },
  { id: 'other',        label: '其他', emoji: '⋯',  color: '#8e8e93', bg: '#f4f5f8' },
];
const INCOME_CATEGORIES = [
  { id: 'salary',     label: '工资', emoji: '💰', color: '#10b981', bg: '#ecfdf5' },
  { id: 'investment', label: '理财', emoji: '📈', color: '#1677ff', bg: '#e6f4ff' },
  { id: 'transfer',   label: '转账', emoji: '🔄', color: '#ff8c00', bg: '#fff7e6' },
  { id: 'bonus',      label: '奖金', emoji: '🎁', color: '#f59e0b', bg: '#fff7e6' },
  { id: 'other',      label: '其他', emoji: '⋯',  color: '#8e8e93', bg: '#f4f5f8' },
];

const NUMPAD_KEYS = [
  { val: '7',   col: 1, row: 1 }, { val: '8', col: 2, row: 1 }, { val: '9', col: 3, row: 1 }, { val: 'del', col: 4, row: 1 },
  { val: '4',   col: 1, row: 2 }, { val: '5', col: 2, row: 2 }, { val: '6', col: 3, row: 2 }, { val: '.',   col: 4, row: 2 },
  { val: '1',   col: 1, row: 3 }, { val: '2', col: 2, row: 3 }, { val: '3', col: 3, row: 3 },
  { val: '0',   col: 1, row: 4, colSpan: 2 }, { val: '00', col: 3, row: 4 },
  { val: '完成', col: 4, row: 3, rowSpan: 2 },
];

const buildDateHelpers = (dateTime) => {
  const dt = dateTime ? new Date(dateTime) : new Date();
  const pad = n => String(n).padStart(2, '0');
  const timeStr = `${pad(dt.getHours())}:${pad(dt.getMinutes())}`;
  const fullDate = `${dt.getFullYear()}年${dt.getMonth()+1}月${dt.getDate()}日 ${timeStr}`;
  const today = new Date(), yday = new Date(today);
  yday.setDate(today.getDate() - 1);
  const DAYS = ['日','一','二','三','四','五','六'];
  const dateLabel = dt.toDateString() === today.toDateString()
    ? `今天 ${dt.getMonth()+1}月${dt.getDate()}日`
    : dt.toDateString() === yday.toDateString()
      ? `昨天 ${dt.getMonth()+1}月${dt.getDate()}日`
      : `${dt.getMonth()+1}月${dt.getDate()}日 星期${DAYS[dt.getDay()]}`;
  return { dt, timeStr, fullDate, dateLabel };
};
...
const useNumpad = (initial = '0') => {
  ...
  const press = (k) => {
    if (k === 'del') {
      ...
      setAmount(p => {
        ...
        if (p.includes('.') && p.split('.')[1]?.length === 1) return p + '0';
        return p + '00';
      });
    } else {
      setAmount(p => {
  ...
  return [amount, setAmount, press];
};

const NumpadGrid = ({ onPress, confirmColor, onConfirm }) => (
  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gridTemplateRows: 'repeat(4,44px)', gap: '8px' }}>
    {NUMPAD_KEYS.map(({ val, col, row, colSpan, rowSpan }) => {
      const isConfirm = val === '完成';
      const isDel = val === 'del';
      return (
        <button
          key={`${col}-${row}`}
          onClick={() => isConfirm ? onConfirm() : onPress(val)}
          style={{
            gridColumn: colSpan ? `${col} / span ${colSpan}` : String(col),
            gridRow: rowSpan ? `${row} / span ${rowSpan}` : String(row),
            ...(isConfirm ? { backgroundColor: confirmColor } : {}),
          }}
          className={`rounded-[12px] text-[18px] font-semibold flex items-center justify-center active:scale-95 transition-transform ${isConfirm ? 'text-white' : isDel ? 'bg-[#ffe0de] text-[#ff3b30]' : 'bg-[#f4f5f8] text-[#1c1c1e]'}`}
        >
          {isDel
            ? <svg viewBox="0 0 24 24" className="w-[18px] h-[18px]" fill="currentColor"><path d="M22 3H7c-.69 0-1.23.35-1.59.88L0 12l5.41 8.11c.36.53.9.89 1.59.89h15c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-3 12.59L17.59 17 14 13.41 10.41 17 9 15.59 12.59 12 9 8.41 10.41 7 14 10.59 17.59 7 19 8.41 15.41 12 19 15.59z"/></svg>
            : val}
        </button>
      );
    })}
...
const AccountListPicker = ({ accounts, selected, onSelect, exclude }) => (
  <div className="mt-[6px] bg-[#f9f9f9] rounded-[14px] p-[8px] space-y-[6px] max-h-[150px] overflow-y-auto hide-scrollbar">
    {accounts.filter(a => !exclude || a.name !== exclude?.name).map((acc, i) => (
      <button key={i} onClick={() => onSelect(acc)}
        className={`w-full flex items-center space-x-[10px] px-[10px] py-[8px] rounded-[10px] border transition-colors ${selected?.name === acc.name ? 'bg-[#e6f4ff] border-[#1677ff]' : 'bg-white border-[#e5e5ea]'}`}>
        <div className="w-[26px] h-[26px] rounded-full bg-[#f4f5f8] flex items-center justify-center shrink-0">{getIconByString(acc.icon)}</div>
        <span className="flex-1 text-left text-[13px] font-medium text-[#1c1c1e] truncate">{acc.name}</span>
        <span className="text-[11px] text-[#8e8e93] shrink-0">{acc.currency}</span>
        {selected?.name === acc.name && <Check className="w-[14px] h-[14px] text-[#1677ff] shrink-0" strokeWidth={2.5} />}
      </button>
    ))}
  </div>
);

const RecordModal = ({ isOpen, onClose, accounts, addTransaction }) => {
  const [txType, setTxType] = useState('expense');
  const [amount, setAmount, press] = useNumpad('0');
  ...
  const accent = txType === 'expense' ? '#ff3b30' : '#10b981';

  const formatTime = () => {
    if (!dateTime) return '时间';
    const d = new Date(dateTime);
    const pad = n => String(n).padStart(2, '0');
    const isToday = d.toDateString() === new Date().toDateString();
    return isToday ? `今天 ${pad(d.getHours())}:${pad(d.getMinutes())}` : `${d.getMonth()+1}/${d.getDate()} ${pad(d.getHours())}:${pad(d.getMinutes())}`;
  };

  const handleConfirm = () => {
    const numVal = parseFloat(amount);
    if (!numVal || !selCategory) return;
    const { timeStr, fullDate, dateLabel } = buildDateHelpers(dateTime);
    addTransaction({
      dateLabel, iconBg: 'bg-[#f4f5f8]', iconType: 'landmark',
      title: note || selCategory.label,
      subtitle: selAccount?.name || '',
      tag: selCategory.label, tagType: selCategory.id,
      amount: (txType === 'expense' ? '-' : '+') + numVal.toFixed(2),
      isIncome: txType === 'income',
      time: timeStr, fullDate,
      currency: selAccount?.currency || 'AED',
      paymentMethod: selAccount?.name || '',
      note,
    });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[430px] bg-white rounded-t-[28px] shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
        <div className="flex justify-center pt-[10px]"><div className="w-[36px] h-[4px] bg-[#e5e5ea] rounded-full" /></div>

        {/* Tabs */}
        <div className="flex items-center px-[16px] pt-[8px] pb-[12px]">
          {(['expense', 'income'] as const).map(t => (
            <button key={t} onClick={() => { setTxType(t); setSelCategory(null); }}
              className={`flex-1 py-[8px] rounded-[10px] text-[15px] font-bold mr-[8px] transition-colors ${txType === t ? (t === 'expense' ? 'bg-[#fff0f0] text-[#ff3b30]' : 'bg-[#ecfdf5] text-[#10b981]') : 'text-[#8e8e93]'}`}>
              {t === 'expense' ? '支出' : '收入'}
            </button>
          ))}
          <button onClick={onClose} className="w-[32px] h-[32px] bg-[#f4f5f8] rounded-full flex items-center justify-center active:bg-[#e5e5ea]">
            <X className="w-[16px] h-[16px] text-[#5c5c5e]" strokeWidth={2} />
          </button>
        </div>

        {/* Amount */}
        <div className="px-[20px] pb-[10px] flex items-baseline">
          <span className="text-[16px] font-medium text-[#8e8e93] mr-[6px]">{selAccount?.currency || 'AED'}</span>
          <span className="text-[42px] font-bold leading-none" style={{ color: accent, fontFamily: 'Helvetica Neue,Arial,sans-serif' }}>{amount}</span>
        </div>
        ...
        <div className="px-[16px] pb-[10px]">
          <div className="grid grid-cols-4 gap-[8px]">
            {[
              { key: 'account',  icon: <CreditCard className="w-[14px] h-[14px]" strokeWidth={2} />, label: selAccount ? selAccount.name.slice(0, 5) : '账户' },
              { key: 'category', icon: <TagIcon className="w-[14px] h-[14px]" strokeWidth={2} />,    label: selCategory?.label || '分类' },
              { key: 'time',     icon: <CalendarDays className="w-[14px] h-[14px]" strokeWidth={2} />, label: formatTime() },
            ].map(f => (
              <button key={f.key} onClick={() => setPicker(p => p === f.key ? null : f.key)}
                className={`flex flex-col items-center py-[8px] px-[2px] rounded-[10px] border transition-all active:scale-95 ${picker === f.key ? 'border-[#1677ff] bg-[#f0f6ff]' : 'border-[#f0f0f0] bg-[#f9f9f9]'}`}>
                <span className={picker === f.key ? 'text-[#1677ff]' : 'text-[#8e8e93]'}>{f.icon}</span>
                <span className={`text-[10px] mt-[3px] font-medium truncate w-full text-center leading-tight ${picker === f.key ? 'text-[#1677ff]' : 'text-[#3a3a3c]'}`}>{f.label}</span>
              </button>
            ))}
            <div className="flex flex-col items-center py-[8px] px-[4px] rounded-[10px] border border-[#f0f0f0] bg-[#f9f9f9] overflow-hidden">
              <input value={note} onChange={e => setNote(e.target.value)} placeholder="备注"
                className="w-full text-[11px] text-center text-[#5c5c5e] outline-none bg-transparent placeholder-[#c7c7cc] font-medium" />
            </div>
          </div>
        </div>
        ...
        {picker === 'account' && (
          <div className="mx-[16px] mb-[8px]">
            <AccountListPicker accounts={accounts} selected={selAccount} onSelect={acc => { setSelAccount(acc); setPicker(null); }} />
          </div>
        )}

        {/* Category picker */}
        {picker === 'category' && (
          <div className="mx-[16px] mb-[8px] bg-[#f9f9f9] rounded-[14px] p-[10px]">
            <div className="grid grid-cols-4 gap-[8px]">
              {categories.map(cat => (
                <button key={cat.id} onClick={() => { setSelCategory(cat); setPicker(null); }}
                  className={`flex flex-col items-center py-[10px] rounded-[10px] border transition-all active:scale-95 ${selCategory?.id === cat.id ? 'border-[#1677ff] bg-[#f0f6ff]' : 'border-[#e5e5ea] bg-white'}`}>
                  <div className="w-[28px] h-[28px] rounded-full flex items-center justify-center mb-[4px] text-[15px]" style={{ backgroundColor: cat.bg }}>{cat.emoji}</div>
                  <span className={`text-[11px] font-medium ${selCategory?.id === cat.id ? 'text-[#1677ff]' : 'text-[#5c5c5e]'}`}>{cat.label}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* Time picker */}
        {picker === 'time' && (
          <div className="mx-[16px] mb-[8px] bg-[#f9f9f9] rounded-[14px] p-[14px]">
            <input type="datetime-local" value={dateTime} onChange={e => { setDateTime(e.target.value); setPicker(null); }}
              className="w-full text-[15px] font-medium text-[#1c1c1e] bg-transparent outline-none" />
          </div>
        )}

        {/* Numpad */}
        <div className="px-[16px] pb-[env(safe-area-inset-bottom,16px)]">
          <NumpadGrid onPress={press} confirmColor={accent} onConfirm={handleConfirm} />
        </div>
      </div>
    </div>
  );
...
const TransferModal = ({ isOpen, onClose, accounts, addTransaction }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setDateTime(initDateTime());
      setFromAcc(accounts[0] || null);
      setToAcc(accounts[1] || null);
      setAmount('0');
      setNote('');
      setPicker(null);
    }
  ...
  const handleConfirm = () => {
    const numVal = parseFloat(amount);
    if (!numVal || !fromAcc || !toAcc) return;
    const { timeStr, fullDate, dateLabel } = buildDateHelpers(dateTime);
    addTransaction({
      dateLabel, iconBg: 'bg-[#5c8af0]', iconType: 'landmark',
      title: `${fromAcc.name} → ${toAcc.name}`,
      subtitle: fromAcc.name,
      tag: '转账', tagType: 'transfer',
      amount: `-${numVal.toFixed(2)}`,
      isIncome: false,
      time: timeStr, fullDate,
      currency: fromAcc.currency || 'AED',
      paymentMethod: fromAcc.name,
      note: note || `${fromAcc.name} 转 ${toAcc.name}`,
    });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[430px] bg-white rounded-t-[28px] shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
        <div className="flex justify-center pt-[10px]"><div className="w-[36px] h-[4px] bg-[#e5e5ea] rounded-full" /></div>
        <div className="flex items-center justify-between px-[20px] pt-[10px] pb-[14px] border-b border-[#f4f5f8]">
          <h2 className="text-[18px] font-bold text-[#1c1c1e]">转账</h2>
          <button onClick={onClose} className="w-[32px] h-[32px] bg-[#f4f5f8] rounded-full flex items-center justify-center active:bg-[#e5e5ea]">
            <X className="w-[16px] h-[16px] text-[#5c5c5e]" strokeWidth={2} />
          </button>
        </div>

        <div className="px-[20px] py-[14px] space-y-[10px]">
          {/* Amount */}
          <div className="flex items-baseline pb-[10px] border-b border-[#f4f5f8]">
            <span className="text-[16px] font-medium text-[#8e8e93] mr-[6px]">{fromAcc?.currency || 'AED'}</span>
            <span className="text-[40px] font-bold text-[#8b5cf6] leading-none" style={{ fontFamily: 'Helvetica Neue,Arial,sans-serif' }}>{amount}</span>
          </div>

          {/* From account */}
          <div>
            <label className="text-[12px] text-[#8e8e93] font-medium block mb-[6px]">从账户</label>
            <button onClick={() => setPicker(p => p === 'from' ? null : 'from')}
              className={`w-full flex items-center justify-between px-[14px] py-[11px] rounded-[14px] border transition-all ${picker === 'from' ? 'border-[#1677ff] bg-[#f0f6ff]' : 'border-[#e5e5ea] bg-[#f9f9f9]'}`}>
              {fromAcc ? (
                <div className="flex items-center space-x-[10px]">
                  <div className="w-[28px] h-[28px] rounded-full bg-[#f4f5f8] flex items-center justify-center">{getIconByString(fromAcc.icon)}</div>
                  <div className="text-left"><div className="text-[14px] font-semibold text-[#1c1c1e]">{fromAcc.name}</div><div className="text-[11px] text-[#8e8e93]">{fromAcc.balance} {fromAcc.currency}</div></div>
                </div>
              ) : <span className="text-[14px] text-[#8e8e93]">选择账户</span>}
              <ChevronDown className={`w-[16px] h-[16px] transition-transform ${picker === 'from' ? 'text-[#1677ff] rotate-180' : 'text-[#c7c7cc]'}`} strokeWidth={2} />
            </button>
            {picker === 'from' && <AccountListPicker accounts={accounts} selected={fromAcc} onSelect={a => { setFromAcc(a); setPicker(null); }} exclude={toAcc} />}
          </div>

          {/* Swap */}
          <div className="flex justify-center">
            <button onClick={() => { const tmp = fromAcc; setFromAcc(toAcc); setToAcc(tmp); }}
              className="w-[32px] h-[32px] bg-[#8b5cf6] rounded-full flex items-center justify-center shadow-md active:scale-95 transition-transform">
          ...
          {/* To account */}
          <div>
            <label className="text-[12px] text-[#8e8e93] font-medium block mb-[6px]">到账户</label>
            <button onClick={() => setPicker(p => p === 'to' ? null : 'to')}
              className={`w-full flex items-center justify-between px-[14px] py-[11px] rounded-[14px] border transition-all ${picker === 'to' ? 'border-[#1677ff] bg-[#f0f6ff]' : 'border-[#e5e5ea] bg-[#f9f9f9]'}`}>
              {toAcc ? (
                <div className="flex items-center space-x-[10px]">
                  <div className="w-[28px] h-[28px] rounded-full bg-[#f4f5f8] flex items-center justify-center">{getIconByString(toAcc.icon)}</div>
                  <div className="text-left"><div className="text-[14px] font-semibold text-[#1c1c1e]">{toAcc.name}</div><div className="text-[11px] text-[#8e8e93]">{toAcc.balance} {toAcc.currency}</div></div>
                </div>
              ) : <span className="text-[14px] text-[#8e8e93]">选择账户</span>}
              <ChevronDown className={`w-[16px] h-[16px] transition-transform ${picker === 'to' ? 'text-[#1677ff] rotate-180' : 'text-[#c7c7cc]'}`} strokeWidth={2} />
            </button>
            {picker === 'to' && <AccountListPicker accounts={accounts} selected={toAcc} onSelect={a => { setToAcc(a); setPicker(null); }} exclude={fromAcc} />}
          </div>

          {/* Time */}
          <div className="flex items-center justify-between px-[14px] py-[11px] rounded-[14px] border border-[#e5e5ea] bg-[#f9f9f9]">
            <div className="flex items-center space-x-[8px]">
              <CalendarDays className="w-[16px] h-[16px] text-[#8e8e93]" strokeWidth={2} />
              <span className="text-[13px] font-medium text-[#5c5c5e]">时间</span>
            </div>
            <input type="datetime-local" value={dateTime} onChange={e => setDateTime(e.target.value)}
              className="text-[13px] font-medium text-[#1c1c1e] bg-transparent outline-none text-right" />
          </div>

          {/* Note */}
          <div className="flex items-center space-x-[8px] px-[14px] py-[11px] rounded-[14px] border border-[#e5e5ea] bg-[#f9f9f9]">
            <Pen className="w-[16px] h-[16px] text-[#8e8e93] shrink-0" strokeWidth={2} />
            <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="备注（可选）"
              className="flex-1 text-[13px] font-medium text-[#1c1c1e] bg-transparent outline-none placeholder-[#c7c7cc]" />
          </div>
        </div>

        {/* Numpad */}
        <div className="px-[16px] pb-[env(safe-area-inset-bottom,16px)]">
          <NumpadGrid onPress={press} confirmColor="#8b5cf6" onConfirm={handleConfirm} />
        </div>
      </div>
    </div>
  );
...
const HomePage = ({ setIsMessageCenterOpen, transactions, accounts, addTransaction }) => {
    ...
    <div className="bg-[#f4f5f8] font-sans text-gray-900 pb-[100px] relative overflow-x-hidden animate-in fade-in duration-300">
      ...
      <div className="px-[16px] space-y-[14px]">
        ...
        <div className="flex justify-between items-center px-[16px] py-[8px]">
          <button onClick={() => setIsRecordOpen(true)} className="flex flex-col items-center space-y-[8px] active:scale-95 transition-transform"><div className="w-[46px] h-[46px] bg-[#1677ff] rounded-full flex items-center justify-center shadow-[0_6px_16px_rgba(22,119,255,0.25)]"><PenLine className="w-[20px] h-[20px] text-white" strokeWidth={2} /></div><span className="text-[12px] font-medium text-[#1c1c1e]">记一笔</span></button>
          ...
          <button onClick={() => setIsTransferOpen(true)} className="flex flex-col items-center space-y-[8px] active:scale-95 transition-transform"><div className="w-[46px] h-[46px] bg-[#8b5cf6] rounded-full flex items-center justify-center shadow-[0_6px_16px_rgba(139,92,246,0.25)]"><ArrowRightLeft className="w-[20px] h-[20px] text-white" strokeWidth={2} /></div><span className="text-[12px] font-medium text-[#1c1c1e]">转账</span></button>
      ...
      <RecordModal isOpen={isRecordOpen} onClose={() => setIsRecordOpen(false)} accounts={accounts} addTransaction={addTransaction} />
      <TransferModal isOpen={isTransferOpen} onClose={() => setIsTransferOpen(false)} accounts={accounts} addTransaction={addTransaction} />
    </div>
  );
};
...
const AssetsPage = ({ setIsMessageCenterOpen, accounts, updateAccount }) => {
        ...
        <div className="fixed inset-0 z-[100] flex justify-center items-end">
          ...
          <div className="relative bg-white w-full max-w-[430px] rounded-t-[24px] pb-[32px] pt-[8px] px-[20px] shadow-2xl animate-in slide-in-from-bottom-8 duration-300 ease-out">
            ...
            <div className="flex space-x-[12px] pb-[8px]">
               ...
               <button onClick={() => { updateAccount(selectedAccount.id || selectedAccount.name, { balance: accountBalance, currency: selectedCurrency }); setIsAccountDetailModalOpen(false); }} className="flex-1 py-[14px] rounded-[14px] text-[16px] font-bold text-white bg-[#1677ff] active:bg-[#0f60d6] transition-colors shadow-[0_4px_12px_rgba(22,119,255,0.25)]">保存修改</button>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  const { accounts, transactions, loading, updateTransaction, updateAccount, addTransaction } = useSupabaseData();
      ...
      <div className="app-container shadow-2xl">
        <div className="scroll-area hide-scrollbar">
            {loading ? (
              ...
              <>
                {activeTab === 'home' && <HomePage setIsMessageCenterOpen={setIsMessageCenterOpen} transactions={transactions} accounts={accounts} addTransaction={addTransaction} />}
                ...
                {activeTab === 'assets' && <AssetsPage setIsMessageCenterOpen={setIsMessageCenterOpen} accounts={accounts} updateAccount={updateAccount} />}
              </>
            )}
        </div>
